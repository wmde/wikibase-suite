...
`entity-graph-dump` bootstraps QLever with one named graph per entity. It
...
`updater` polls Wikibase Recent Changes and performs **entity RDF
...
The updater writes its cursor atomically to `wdqs-qlever-updater-state.json`, its
health to `wdqs-qlever-updater-health.json`, and observable endpoint progress to the
`urn:wikibase-suite:wdqs-qlever-updater-state` graph. A failed request leaves the
cursor unchanged and uses bounded exponential retry; the next attempt safely
replaces the same entity graph again.

On startup, after bootstrap, and after a failed polling cycle it verifies that
the cursor is still within Wikibase's Recent Changes retention window. It
refuses to advance silently if history has
expired; run `reconcile --all --repair` (then bootstrap to
...
updater acknowledges it and pauses. A separate advisory update lock lets the
exporter wait for any in-flight write, including when the updater exits without
acknowledging the pause. The indexer removes the bootstrap marker only after the
replacement index succeeds; the updater then reloads the export's high-water
cursor. This keeps the updater paused across both dump and indexing, so it
cannot write updates that a replacement index would erase. Do
...
operation, stop `wdqs-qlever-updater` and `qlever` before starting the dump/index
...
`wikibase/wdqs-qlever-updater` images. The Query Service image contains the tested
...
BUILD_CACHE_REGISTRY='' ./wbs-dev build wdqs-qlever-updater
```
...
This fixture evaluates Recent Changes polling without a streaming platform.
...
Use `wbs install --from-source` to build and start the native images. It
generates `.wbs/local-images.override.yml`; include that generated override in
subsequent manual Compose commands:

```sh
docker compose -p wbs-deploy \
	-f docker-compose.yml \
	-f .wbs/local-images.override.yml up -d
```

When testing a rebuilt updater image, recreate only that service; `--no-deps`
is important because the test overlay's dependency chain includes the
static-index one-shot services (root Compose does not):

```sh
docker compose -p wbs-deploy \
	-f docker-compose.yml \
	-f .wbs/local-images.override.yml \
...
docker compose -p wbs-deploy \
	-f docker-compose.yml \
	-f .wbs/local-images.override.yml \
  exec -T wdqs-qlever-updater verify Q1 P25
```
...
docker compose -p wbs-deploy \
	-f docker-compose.yml \
	-f .wbs/local-images.override.yml \
  exec -T wdqs-qlever-updater reconcile --repair Q1 P25
```
