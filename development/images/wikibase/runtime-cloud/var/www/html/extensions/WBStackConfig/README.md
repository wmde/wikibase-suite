...
The Cloud image selects `/opt/wikibase-cloud/CloudSettings.php` through `MW_CONFIG_FILE`. That entry point loads this package; `PLATFORM_API_BACKEND_HOST` supplies the lookup API host. The Suite variant packages its own configuration instead.
...
The PHP and assets originate from `wbstack/mediawiki` revision `220017feecaf1332086d0efc4518796916ead509`. This package includes an initial MediaWiki 1.46 adaptation described below; it is not a verbatim upstream snapshot.
